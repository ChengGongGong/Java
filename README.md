...
![image](https://user-images.githubusercontent.com/41152743/147639946-5a9c16aa-0996-43b6-b9d4-b08be4d216a9.png)
                        ...
                        3. 并发重分配：把重分配集中的存活对象复制到新的Region上，并为重分配集中的每个Region维护一个转发表，记录从旧对象到新对象的转向关系。
                            如果用户线程并发访问了位于重分配集中的对象，这次访问将会被预置的内存屏障所截获，然后立即根据Region上的转发表记录将访问转发到新复制的对象
上，并同时修正更新该引用的值，使其直接指向新对象。
                        4. 并发重映射：是修正整个堆中指向重分配集中旧对象的所有引用，该动作合并到下一次垃圾收集循环中的并发标记阶段里去完成，都是需要遍历所有对象。一旦所有指针都被修正之后，原来记录新旧对象关系的转发表就可以释放掉了
