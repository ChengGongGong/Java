...
![image](https://user-images.githubusercontent.com/41152743/147624951-c38e87fa-3678-4822-97ef-a7a721c0cdd9.png)
            ...
            6. 比较：
                1. 相对于CMS，G1的内存占用更高，因为每个Region都维护有自己的记忆集，CMS的卡表只有一份，处理老年代到新生代的引用，因此G1的内存占用可能会占整个堆容量的20%乃至更多的内存空间。
                2. 相对于CMS，G1的执行负载更高，CMS使用写后屏障来更新维护卡表，G1使用写后屏障来更新维护卡表，另外使用写前屏障来跟踪并发时的指针变化情况。

                3. 目前而言，小内存应用上CMS的表现大概率仍然要会优于G1，而在大内存应用上G1则大多能发挥其优势，java堆容量的平衡点通常在6GB至8GB之间，随着开发者对G1的不断优化，会更倾向于G1.
    
            7. 低延迟垃圾收集器
                1. Shenandoah收集器：由OracleJDK排除的收集器，只存在于OpenJDK中，与G1类似，管理堆内存方面的优化：
                    1. 支持并发的整理算法，G1的回收阶段是可以多线程并行的，但不能与用户线程并发；
                    2. 默认不使用分代收集，不会有专门的新生代Region或者老年代Region的存在，没有实现分代；
                    3. 采用连接矩阵的全局数据结构来记录跨region的引用关系
                2. ZGC收集器：JDK 11加入，在尽可能对吞吐量影响不太大的前提下，实现在任意堆内存大小下都可以把垃圾收集的停顿时间限制在十毫秒以内的低延迟；
                    1. 基于Region内存布局的，（暂时）不设分代的，使用了读屏障、染色指针和内存多重映射等技术来实现可并发的标记-整理算法，以低延迟为首要目标
                    2. Region分类：
                    ...
                    3. 染色体技术：直接把标记信息记在引用对象的指针上  
![image](https://user-images.githubusercontent.com/41152743/147639036-a27a2d1b-6a2b-4790-80d3-6b4c7c851b8c.png)

                        1. Linux下64位指针的高18位不能用来寻址，但剩余的46位指针所能支持的64TB内存在今天仍然能够充分满足大型服务器的需要，染色体技术基于此46位指针。
                        2. 将其高四位提取存储四个标志信息，能够管理的内存不可以超过4TB（2的42次幂），不支持32位平台，不支持压缩指针
                        3. 染色体指针优势：
                            1. 可以使得一旦某个Region的存活对象被移走之后，这个Region立即就能够被释放和重用掉，而不必等待整个堆中所有指向该Region的引用都被修正后才能清理。
                            2. 可以大幅减少在垃圾收集过程中内存屏障的使用数量，到目前为止ZGC都并未使用任何写屏障，只使用了读屏障；
                            3. 可以作为一种可扩展的存储结构用来记录更多与对象标记、重定位过程相关的数据
                    4.多重映射
                        Linux/x86-64平台上的ZGC使用了多重映射（Multi-Mapping）将多个不同的虚拟内存地址映射到同一个物理内存地址上
                    5. 运行过程：
![image](https://user-images.githubusercontent.com/41152743/147639946-5a9c16aa-0996-43b6-b9d4-b08be4d216a9.png)
                   
                        1. 并发标记：遍历对象图做可达性分析的阶段，在指针上而不是在对象上进行的，标记阶段会更新染色指针中的Marked 0、Marked 1标志位。
                        2. 并发预备重分配：根据特定的查询条件统计得出本次收集过程要清理哪些Region，将这些Region组成重分配集，重分配集只是决定了里面的存活对象会被重新复制到其他的Region中，里面的Region会被释放，回收行为是每次都会扫描所有的region。
                        3. 并发重分配：把重分配集中的存活对象复制到新的Region上，并为重分配集中的每个Region维护一个转发表，记录从旧对象到新对象的转向关系。
                            如果用户线程并发访问了位于重分配集中的对象，这次访问将会被预置的内存屏障所截获，
                            然后立即根据Region上的转发表记录将访问转发到新复制的对象上，并同时修正更新该引用的值，使其直接指向新对象。
                        4. 并发重映射：是修正整个堆中指向重分配集中旧对象的所有引用，该动作合并到下一次垃圾收集循环中的并发标记阶段里去完成，都是需要遍历所有对象。 
                            一旦所有指针都被修正之后，原来记录新旧对象关系的转发表就可以释放掉了
    
    3. 内存分配与回收策略
    
        1. 对象优先在Eden分配：当Eden区没有足够空间进行分配时，虚拟机将发起一次Minor GC。
        2. 大对象直接进入老年代：大对象就是指需要大量连续内存空间的Java对象，-XX：PretenureSizeThreshold参数，指定大于该设置值的对象直接在老年代分配，
              最典型的大对象便是那种很长的字符串，或者元素数量很庞大的数组
        ...
        5. 空间分配担保：
            ...
            2. 如果不成立会先查看-XX：HandlePromotionFailure参数的设置值是否允许担保失败（Handle Promotion Failure）
            3. 如果允许，那会继续检查老年代最大可用的连续空间是否大于历次晋升到老年代对象的平均大小，如果大于，将尝试进行一次Minor GC
            4. 如果小于，或者-XX：HandlePromotionFailure设置不允许冒险，那这时就要改为进行一次Full GC。
            注：JDK 1.6后，实际虚拟机不再使用-XX：HandlePromotionFailure参数，
            规则变更为：只要老年代的连续空间大于新生代对象总大小或者历次晋升的平均大小，就会进行Minor GC，否则将进行Full GC。
    
## 2. 内存泄露与内存溢出
    
### 1. 内存泄露——纯代码层面的问题，完善代码来避免
    程序在申请内存后，无法释放已申请的内存空间，一次内存泄露危害可以忽略，但内存泄露堆积后果严重，导致内存溢出。
    
    1. 内存泄露就是存在一些被分配的对象，不会被GC回收，但依旧占用内存，特点：
        
        1. 对象是可达的，即在有向图中，存在通路可以与其相连；
        2. 对象是无用的，程序以后不会再使用这些对象。
    2. 常见的情形：
        
        1. 静态的集合类，生命周期同程序一致，在程序结束前不能被释放。
        2. 各种连接，如数据库连接、网络连接和IO连接，这些连接如果不显性的关闭，将会造成大量的对象无法被回收，从而引起内存泄露；
        3. 变量不合理的作用域，一个变量的定义的作用范围大于其使用范围，很有可能造成内存泄露，且没有及时把对象设置为null，也有可能导致内存泄露的发生；
        4. 内部类持有外部类，一个外部类的实例对象的方法返回了一个内部类的实例对象，这个内部类被长期引用了，即使外部类实例对象不再被使用，由于内部类持有外部类的实例对象，外部类对象不会被垃圾回收。
        5. 改变哈希值，当一个对象被存储进hashset集合以后，就不能修改这个对象中参与计算哈希值的字段了，否则对象修改后的哈希值与最初存进去的哈希值不同，无法单独删除当前对象。
        6. 过期的引用，栈先增长再收缩，从栈中弹出的对象不会被当作垃圾回收，即使程序不再使用栈中的那些对象，因为栈中仍保存着这对象的引用；
        7. 缓存泄露
        8. 监听器和回调，在API中注册回调，却没有显示的取消，就会聚集。
    3. 典型的排查案例
        
        1. 使用jps查看当前JVM中运行的进程状态信息，jps -l
        2. 使用jstat命令查看虚拟机各种运行状态统计信息，jstat -gcutil 监视java堆情况；
        3. jmap用于生成堆转储快照(一般称为heap dump或者dump文件)，
            1. jmap -dump:format=b,file=dumpFileName pid 命令将把堆dump下来使用MAT、VisualVM等工具分析，
